...
class CVSplmAutomation:
    ...
    def on_generate_pdf_click(self, b: widgets.Button) -> None:
        ...
        with self.output:
            ...
            part_identifier = (
                self.part_number
                if self.validation_type != ValidationType.SCHEDULE.value
                else self.schedule_dropdown.value
            )
            # Convert validation_type to ValidationType enum
    ...
    def search_part(self) -> bool:
        ...
        if search:
            ...
            # Map the string to LifeCycleStatus enum value
            try:
    ...
    def execute_schedule(self) -> None:
        ...
        with tqdm(
            ...
            try:
                self.validation_type = ValidationType.SCHEDULE.value
                search = self.start_schedule_interaction(
                    progress_bar, process_name=self.validation_type
                )
                if search:
    ...
    def execute_ini_automation(self):
        ...
        with tqdm(
            ...
            try:
                self.validation_type = ValidationType.INI.value
